...
def result2():
    ...
    gen_means = gen_means.append(
        {'random': gen_shs27_random.mean().values[0], 'BFS': gen_shs27_bfs.mean().values[0],
         'DFS': gen_shs27_dfs.mean().values[0]}, ignore_index=True)
    gen_means = gen_means.append(
        {'random': gen_shs148_random.mean().values[0], 'BFS': gen_shs148_bfs.mean().values[0],
...
def results_pipr():
    shs27_bfs = pd.read_csv('save_test_results/PIPR_SHS27k_bfs', sep=',', header=None).iloc[:, 3:]
    shs27_dfs = pd.read_csv('save_test_results/PIPR_SHS27k_dfs', sep=',', header=None).iloc[:, 3:]
    shs27_random = pd.read_csv('save_test_results/PIPR_SHS27k_random', sep=',', header=None).iloc[:, 3:]

    shs148_bfs = pd.read_csv('save_test_results/PIPR_SHS148k_bfs', sep=',', header=None).iloc[:, 3:]
    shs148_dfs = pd.read_csv('save_test_results/PIPR_SHS148k_dfs', sep=',', header=None).iloc[:, 3:]
    shs148_random = pd.read_csv('save_test_results/PIPR_SHS148k_random', sep=',', header=None).iloc[:, 3:]

    cols = ['X_all', 'X_bs', 'X_es', 'X_ns']
    means = pd.DataFrame(columns=cols)
    stds = pd.DataFrame(columns=cols)
    for table in [shs27_random, shs27_bfs, shs27_dfs, shs148_random, shs148_bfs, shs148_dfs]:
        table[3] = table[3].apply(lambda x: float(x.split(': ')[1]))
        table[4] = table[4].replace(' None', -1)
        means = means.append({k: v for k, v in zip(cols, table.mean())}, ignore_index=True)
        stds = stds.append({k: v for k, v in zip(cols, table.std())}, ignore_index=True)
...
if __name__ == '__main__':
    # call functions from above, depending on which results you would like to combine
    # functions print skeletons for the latex tables used in the report
    results_pipr()
